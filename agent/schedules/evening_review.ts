...
import { localNow, tomorrowInTz } from "../lib/time.js";
...
export default defineSchedule({
  ...
  async run({ receive, waitUntil, appAuth }) {
    ...
    const workLine = summarizeWorkDay(await workDayInfo(tomorrowInTz(tz)), "tomorrow");
    waitUntil(
      receive(telegram, {
        message:
          `Current time: ${now} (${tz}). ` +
          (workLine ? `Tomorrow's work context (background only — do NOT restate it): ${workLine} ` : "") +
          ...
          "tomorrow. Don't state his work status outright; use the work context only to flag " +
          "something actionable about tomorrow if there is any (an early meeting, an in-office " +
          "day worth prepping for tonight). Two to four bullets.",
        target: { chatId },
        auth: appAuth,
      }),
    );

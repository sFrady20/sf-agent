...
import { isValidTimeZone, ownerTimezone, stripOffset } from "../lib/time.js";
...
// offset the model attached is discarded. Defaults to his home zone, but accepts
// an explicit `timezone` for events scheduled elsewhere (e.g. while traveling).
export default defineTool({
  description:
    "Create a Google Calendar event. Call current_time first, then confirm the title, date, and time with Steven before calling. Pass the local wall-clock time (an offset is optional and will be recomputed). Defaults to Steven's home timezone; pass `timezone` when the event is in a different zone, e.g. while traveling.",
  inputSchema: z.object({
    ...
    timezone: z
      ...
      .optional()
      .describe(
        "IANA timezone the wall-clock time is in, e.g. 'America/Los_Angeles'. Defaults to Steven's home timezone.",
      ),
  }),
  needsApproval: once(),
  async execute({ summary, start, end, description, location, timezone }) {
    const tz = timezone ?? ownerTimezone();
